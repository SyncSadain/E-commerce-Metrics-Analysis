...
SELECT products.product_name AS Product, products.price AS Price, RANK() OVER (ORDER BY products.price DESC) AS Price_Rank
FROM products
ORDER BY Price_Rank
LIMIT 5;
...
SELECT customers.customer_name AS Customers, SUM(orders.total_amount) AS Total_Spending, RANK() OVER ( ORDER BY sum(orders.total_amount) DESC) AS Highest_Spenders
FROM customers
JOIN orders ON orders.customer_id = customers.customer_id
...
FROM customers
JOIN orders ON orders.customer_id = customers.customer_id
WHERE orders.total_amount > (SELECT AVG(orders.total_amount)
FROM orders)
...
SELECT ROW_NUMBER() OVER (ORDER BY customers.customer_name) AS Row_Num, customers.customer_name AS Customer, products.product_name AS Product, orderdetails.quantity AS Quantity
FROM customers
JOIN orders ON orders.customer_id = customers.customer_id
JOIN orderdetails ON orderdetails.order_id = orders.order_id
JOIN products ON products.product_id = orderdetails.product_id
...
SELECT customers.customer_name AS Customer, COUNT(orders.order_id) AS Total_Orders
FROM customers
JOIN orders ON orders.customer_id = customers.customer_id
GROUP BY customers.customer_name
...
SELECT orderdetails.quantity, customers.customer_city, products.product_name
FROM products
JOIN customers ON orderdetails.product_id = products.product_id;
